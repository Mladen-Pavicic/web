...
#define PEVAL
#if defined(PEVAL)
void partialEval(vstring trialEqn, vstring varFlags, long *eqnLen);
#else
#endif
...
unsigned char test(void)
{
  ...
  vstring trialHypListP[MAX_HYPS][MAX_VARS];
  vstring trialConclusionP[MAX_VARS];
  ...
  for (i = 0; i <= maxDepth + 1; i++) {
    for (j = 0; j < hypotheses; j++) {
      ...
      let(&(trialHypListP[j][i]), polHypList[j]);
      let(&(unassignedHypVarsP[j][i]), space(hypLen[j]));
    }
    trialConclusionP[i] = "";
    unassignedConclVarsP[i] = "";
    let(&(trialConclusionP[i]), trialConclusion);
  ...
  while (1) {
    if (loopVar[loopDepth] < nodes) { /* Loop not exhausted */
      ...
      if (e == FALSE_CONST) goto nextIter;  /* A hypothesis failed;
                                               next iteration */
      if (e != TRUE_CONST) bug(240);
#if defined(PEVAL)
...
#define FUDGE_PARAMETER 0
      if (loopDepth < maxDepth - FUDGE_PARAMETER) {
        /* Do a partial evaluation using variables assigned so far
           to reduce the length of the equation. */
/*D*/if(0)printf("b%ld %ld %s %s\n",loopDepth,maxDepth,tmptrialeq1,tmpuav1);
        partialEval(tmptrialeq1, tmpuav1,
...
/*D*/if(0)printf("a%ld %ld %s %s\n",loopDepth,maxDepth,tmptrialeq1,tmpuav1);
      ...
      if (loopDepth < maxDepth) {
        ...
        if (totB < LONG_MAX - conclusionLen) { /* Don't add if overflow */
...
/*????*****THIS DOES NOT WORK WITH NEW BRENDAN NOTATION!!! ********/
  ...
  for (i = 0; i <= maxDepth; i++) {  /* Deallocate strings */
    for (j = 0; j < hypotheses; j++) {
      let(&(trialHypListP[j][i]), "");
      let(&(unassignedHypVarsP[j][i]), "");
    }
    let(&(trialConclusionP[i]), "");
    ...
    if (e == NEG_OPER) {
      ...
      if (e == NOT_OPER) {
        if (stack[stackPtr] == TRUE_CONST) {
          ...
          if (stack[stackPtr] != FALSE_CONST) bug(251);
...
void partialEval(vstring trialEqn, vstring unassignedVarFlags,
    long *eqnLen)
{
  unsigned char stack[MAX_STACK2];
  long stackSubexprLen[MAX_STACK2]; /* Has unevaluated subexpr len for stack */
  long stackUnevalFlag[MAX_STACK2]; /* Has uneval (=2) flags for stack */
  unsigned char e, e1;
...
/*D*/}
    ...
    if (e1 < 127) { /* It is a binary operator */
      ...
      if (stackUnevalFlag[stackPtr] != 'n'
        ...
        if (stackUnevalFlag[stackPtr - l1] == 'n') {
          if (l2 != 1) bug(314);
          if (stack[stackPtr - l1] == '0') { /* 2nd arg is 0 */
            if (e == '^') {
              /* Result is 0 */
              stackPtr = stackPtr - l1 - l2 + 1;
              stack[stackPtr] = '0';
          ...
          if (stack[stackPtr - l1] == '1') {  /* 2nd arg is 1 */
            if (e == '^') {
              /* Remove the 1 */
              for (j = stackPtr - l1; j < stackPtr; j++) {
                stack[j] = stack[j + 1];
    ...
    /* Could comment out bugcheck for ~5% speedup - but would be dangerous
       if input expression too long - future: we could pre-check this */
    if (stackPtr >= MAX_STACK2) bug(309);
    stack[stackPtr] = e;
